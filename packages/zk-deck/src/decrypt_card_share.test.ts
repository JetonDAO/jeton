...
describe("decrypt card share", () => {
  ...
  test("should prove and verify decrypt_card_share", async () => {
    ...
    const publicKey = curve.pointToStringTuple(
      curve.mulScalarPoint(secretKey, curve.generator),
    );
    const inputPointValue = curve.mulScalarPoint(
      curve.sampleScalar(),
      curve.generator,
    );
    ...
    expect(
      await verifyDecryptCardShare(publicKey, inputPoint, outputPoint, proof),
    ).to.be.true;
  });
